...
export class Transactions extends Component {
	...
	pageToAfter = (page, pageSize) =>
		this.wholePaging ? page * pageSize : this.state.pages[page];

	onPageSizeChange = (pageSize, page) => {
		if (this.wholePaging) {
			this.refetch(
				{ after: this.pageToAfter(page, pageSize), pageSize },
				{ page }
			);
		} else {
	...
	onPageChange = page => {
		this.refetch(
			{ after: this.pageToAfter(page, this.props.pageSize) },
			{ page }
		);
	};
	...
	creatorIdOnChange(creatorId) {
		this.refetch(
			{ after: null, creatorId },
			{ page: 0, pages: [null], creatorId }
		);
	}

	get wholePaging() {
		return (
			this.props.timeAfter === null &&
			this.props.timeBefore === null &&
	...
	render() {
		...
		const { transaction, dialogOpen, page, pages, creatorId } = this.state;
		return (
			<div>
					...
					pages={
						this.wholePaging
							? totalTransactionCount !== null
								? Math.ceil(totalTransactionCount / pageSize)
								: 1
							: pages.length
					}
				/>
		...
		`,
		{
			...
			props({
				data: {
					...
					loading,
					refetch,
					variables: { pageSize, timeAfter, timeBefore }
				}
			}) {
				return {
					transactionList: list
						? list.items.map(({ hash, time, createdBy }) => ({
								txhash: hash,
								createdt: time,
